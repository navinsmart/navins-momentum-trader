...
- Fractional shares
- Crash Protection with Hysteresis:
   - Exit to cash when QQQ < 100-day MA
   - Reinvest only when QQQ > 55-day MA
"""
...
EXIT_MA_DAYS = 100          # Sell when QQQ goes below 100-day MA
REENTRY_MA_DAYS = 55        # Reinvest only when QQQ goes back above 55-day MA
...
def sell_everything():
   """Sell all current positions and go to cash"""
   positions = get_current_positions()
   if not positions:
       print("Already in cash.")
       return

   print("Selling all positions (going to cash)...")
...
# ============================================================
# MARKET TREND FILTER (Hysteresis)
# ============================================================
def is_market_healthy():
   """
   Hysteresis Market Filter:
   - Exit to cash when QQQ < 100-day MA
   - Re-enter only when QQQ > 55-day MA
   """
   if not USE_MARKET_TREND_FILTER:
       return True

   qqq = yf.download("QQQ", period="1y", auto_adjust=True, progress=False)["Close"]
   if isinstance(qqq, pd.DataFrame):
       qqq = qqq.iloc[:, 0]

   if len(qqq) < EXIT_MA_DAYS:
       print("Not enough data for market filter – allowing trades")
       return True

   current = float(qqq.iloc[-1])
   ma_exit = float(qqq.rolling(EXIT_MA_DAYS).mean().iloc[-1])      # 100-day
   ma_reentry = float(qqq.rolling(REENTRY_MA_DAYS).mean().iloc[-1]) # 55-day

   print(f"QQQ Current Price:   {current:.2f}")
   print(f"100-day MA (Exit):   {ma_exit:.2f}")
   print(f"55-day MA (Reentry): {ma_reentry:.2f}")
   ...
   if currently_invested:
       # We are invested → only exit if below 100-day MA
       if current < ma_exit:
           print("→ QQQ below 100-day MA → SELL EVERYTHING")
           return False
       else:
           print("→ Still healthy (above 100-day MA)")
           return True
   else:
       # We are in cash → only re-enter if above 55-day MA
       if current > ma_reentry:
           print("→ QQQ back above 55-day MA → ALLOW REINVESTING")
           return True
       else:
           print("→ Still below 55-day MA → STAY IN CASH")
           return False
...
if __name__ == "__main__":
   ...
   print(f"Exit when QQQ < {EXIT_MA_DAYS}-day MA")
   print(f"Re-enter when QQQ > {REENTRY_MA_DAYS}-day MA")
   rebalance()
